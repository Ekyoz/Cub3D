...
DIR_LIST			= $(FILE_XXX_DIR)

SRC_FILES			+= $(addprefix $(FILE_XXX_DIR), $(FILE_XXX))
...
$(SRC_OUT_DIR)/%.o: $(SRC_DIR)/%.cpp $(HEADERS) Makefile | $(OBJF)
			$(call compile_object,$(CFLAGS))

$(DEBUG_OUT_DIR)/%.o: $(SRC_DIR)/%.cpp $(HEADERS) Makefile | $(OBJF)
			$(call compile_object,$(CFLAGS_DEBUG))

$(TEST_OUT_DIR)/%.o: $(TEST_DIR)/%.cpp $(HEADERS) Makefile | $(OBJF)
